...
function Map(elem) {
    ...
    $(elem).on('mousemove', function (ev) {
		...
		this.lastCursorPositionReal = this.viewportToReal({x: event.offsetX, y: event.offsetY});
    }.bind(this));
...
Map.prototype.drawObjects = function () {
    ...
    for (var i = 0; i < this.objects.length; i++) {
        ...
        var isVisible = objPosReal.x + objRadiusReal >= viewportReal.x - viewportReal.w / 2 &&
                viewportReal.x + viewportReal.w / 2 >= objPosReal.x - objRadiusReal &&
                objPosReal.y + objRadiusReal >= viewportReal.y - viewportReal.h / 2 &&
                viewportReal.y + viewportReal.h / 2 >= objPosReal.y - objRadiusReal;
