...
connectionpool получает соединени и берёт от него авторизацию.
взятие авторизации начато только для вебсокета
	websocket вычитывает из соединения сообщение, и натравливает на него unmarshal для websocketmessagewrapper. это происходит нормально - теперь следующий шаг - нужно реализовать десериализацию сообщений разных типов
	ВСЕГДА ВОЗВРАЩАЕТСЯ NIL ПОСЕЛ ПАРСИНГА

А ЕЩЁ СЕРВЕР ЛОЖИТСЯ ПОСЛЕ КАЖДОГО СОЕДИНЕНИЯ.
