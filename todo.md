...
основной цикл
	если
		пошаговая симуляция
			...
			применяем все инпуты из очереди и делаем шаг симуляции 
		повтор
			ничего не принимаем
...
ОБЩИЙ СЛУЧАЙ ЦИКЛА
	ПОЛУЧАЕМ ИНПУТЫ -> очередь инпутов
		...
		повтор - просто берём все инпуты до следующего шага
		обычный режим - принимаем пока не настанет время симуляции
	ШАГ СИМУЛЯЦИИ 
	    <- очередь инпутов (вот тут должна происходить запись инпутов, чтобы потом можно было увидеть откаты и т.п.)
    ...
    Игровая логика GameLogic {
        ...
        Состояние игры GameState {
            Список состояний игроков []PlayerState {
                ...
                Id соответствующего физического объекта/ов (entity)
            }
...
Итак.
1. YAGNI
2. Каждый кусок должен быть минимальным и тестируемым
3. Применение действий, симуляция, поиск коллизий и их обработка - отдельный пакет, который ничего не знает про время - ему на вход подаётся только Δt. Simulate(d time.Duration)
4. Получение ввода пользователей, откат состояния, рассылка состоянии игрокам и запись состояния в файл - это другой пакет. 
5. Писать в соединение с пользователем может любой сервис. Т.е. в объекте "Пользователь" должен лежать реальный сокет, чтобы не гонять ответы через брокер. Брокер нужен для мультиплексирования сообщений ОТ пользователя в разные сервисы.
6. На "откат" надо пока забить
7. Пробуем использовать ECS - будем оперировать только сущностями и компонентами.
8. Синхронизация списка сущностей с клиентом - отдельная штука. Все сущности должны быть унифицированы. Компоненты можно сделать иммутабельными и хранить в них время создания(оно же изменение), тогда проще будет синхронизировать 
